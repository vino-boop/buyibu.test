...
async function callAI(prompt: string, systemInstruction?: string, isJson = false) {
  ...
  console.log(`[AI-Service] Calling ${config.provider} with model: ${modelName}`);
  ...
  if (config.provider === 'DEEPSEEK') {
    ...
    // 关键修正：deepseek-reasoner 目前不支持 json_object 格式
    ...
    try {
      const response = await fetch(apiUrl, {
        ...
        body: JSON.stringify({
          ...
          temperature: isReasoner ? undefined : 0.7, // Reasoner 通常建议不设置或使用默认值
        })
      });
...
export async function chatWithContext(messages: ChatMessage[], context: string): Promise<string> {
  ...
  请以${isProfessional ? '专业且详尽' : '通俗易懂'}的口吻回答。最后附带一个引导性的追问。`;
...
export async function interpretLiuYao(lines: HexagramLine[], question: string, userProfile?: any): Promise<LiuYaoResponse> {
  ...
  const prompt = `问题：${question}。卦象序列：${lineStr}。请以纯 JSON 格式输出，包含 hexagramName, hexagramSymbol, analysis, judgment 四个字段。不要输出任何解释文字。`;

  try {
    const response = await callAI(prompt, "你是一个专业的六爻解卦师。只返回 JSON 数据，禁止包含 Markdown 代码块或文字说明。", true);
    
    // 使用增强的提取逻辑
    const cleanJson = extractJson(response);
    console.log("[LiuYao] Cleaned JSON:", cleanJson);
    
    return JSON.parse(cleanJson);
  } catch (error: any) {
    console.error("[LiuYao-Parse-Error]", error);
    return { 
      hexagramName: "起卦成功", 
      hexagramSymbol: "☯", 
      analysis: `### 解析受阻\n解析模型输出时出错。原始输出：\n${error.message}` 
