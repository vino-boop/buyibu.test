...
export const BaZiView: React.FC<BaZiViewProps> = ({ defaultQuestion, isDayMode = false }) => {
  ...
  // Extract suggestions from the last message
      ...
      <div ref={scrollContainerRef} className="flex-1 overflow-y-auto scrollbar-hide pb-32 pt-4">
          <div className={`shadow-lg rounded-3xl mb-4 transition-colors ${isDayMode ? 'bg-white border-gray-100' : 'bg-mystic-paper/90 border-white/5'} border mx-4`}>
             {chartDisplayMode === 'COLLAPSED' && (
                <div className="px-6 py-4 flex items-center justify-between">
                   <div className="flex items-center gap-8">
                      ...
                      <div className={`flex flex-col border-l pl-8 transition-colors ${isDayMode ? 'border-gray-100' : 'border-white/10'}`}>
                      ...
                      <div className={`hidden sm:flex flex-col border-l pl-8 transition-colors ${isDayMode ? 'border-gray-100' : 'border-white/10'}`}>
                         <span className="text-[10px] text-gray-400 font-bold uppercase tracking-tighter mb-1">公历生辰</span>
                         <span className={`text-xs ${isDayMode ? 'text-gray-500' : 'text-gray-400'}`}>{chart.solarDate}</span>
                      </div>
                   </div>
             ...
             {chartDisplayMode === 'EXPANDED' && (
                <div className={`p-4 rounded-2xl overflow-hidden animate-fade-in-down transition-colors ${isDayMode ? 'bg-gray-50' : 'bg-black/20'}`}>
                   ...
                   {!showFullDetails && (
                       <div className={`grid grid-cols-5 text-center text-xs py-4 gap-y-3 mb-4 mx-2 rounded-xl border transition-colors ${isDayMode ? 'bg-white border-gray-100' : 'bg-white/5 border-white/5'}`}>
                            <div className="text-gray-500 font-bold flex items-center justify-center">天干</div>
                            {pillars.map((p, i) => <div key={i} className={`text-2xl font-bold font-serif ${getElementColor(p.data.stem, isDayMode)}`}>{p.data.stem}</div>)}
                            <div className="text-gray-500 font-bold flex items-center justify-center">地支</div>
                            {pillars.map((p, i) => <div key={i} className={`text-2xl font-bold font-serif ${getElementColor(p.data.branch, isDayMode)}`}>{p.data.branch}</div>)}
                       </div>
                   )}
                   <div className="px-2">
                       ...
                       <div className="mt-6 animate-fade-in">
                          <div className={`mx-2 rounded-xl border overflow-hidden mb-6 transition-colors ${isDayMode ? 'bg-white border-gray-100' : 'bg-white/5 border-white/5'}`}>
                              <div className={`grid grid-cols-5 text-center text-xs divide-y transition-colors min-w-0 ${isDayMode ? 'divide-gray-100' : 'divide-white/5'}`}>
                                  <div className={`py-3 text-gray-500 font-bold ${isDayMode ? 'bg-gray-50/50' : 'bg-white/5'}`}>位置</div>
                                  {pillars.map((p, i) => <div key={i} className={`py-3 ${isDayMode ? 'text-gray-400 bg-gray-50/50' : 'text-gray-400 bg-white/5'}`}>{p.label}</div>)}
                                  <div className="py-3 text-gray-500 font-bold flex items-center justify-center">十神</div>
                                  {pillars.map((p, i) => <div key={i} className={`py-3 font-medium ${isDayMode ? 'text-gray-700' : 'text-gray-300'}`}>{p.data.mainStar || '-'}</div>)}
                                  <div className={`py-5 text-gray-500 font-bold flex items-center justify-center ${isDayMode ? 'bg-gray-50/50' : 'bg-white/5'}`}>天干</div>
                                  {pillars.map((p, i) => <div key={i} className={`py-5 text-3xl font-bold font-serif ${isDayMode ? 'bg-gray-50/50' : 'bg-white/5'} ${getElementColor(p.data.stem, isDayMode)}`}>{p.data.stem}</div>)}
                                  <div className={`py-5 text-gray-500 font-bold flex items-center justify-center ${isDayMode ? 'bg-gray-50/50' : 'bg-white/5'}`}>地支</div>
                                  {pillars.map((p, i) => <div key={i} className={`py-5 text-3xl font-bold font-serif ${isDayMode ? 'bg-gray-50/50' : 'bg-white/5'} ${getElementColor(p.data.branch, isDayMode)}`}>{p.data.branch}</div>)}
                                  <div className="py-4 text-gray-500 font-bold flex items-center justify-center">藏干</div>
                                  {pillars.map((p, i) => (<div key={i} className="py-4 flex flex-col items-center gap-1">{p.data.hiddenStems.map((hs, idx) => <span key={idx} className={`text-xs font-serif ${getElementColor(hs, isDayMode)}`}>{hs}</span>)}</div>))}
                                  <div className={`py-4 text-gray-500 font-bold flex items-center justify-center ${isDayMode ? 'bg-gray-50/50' : 'bg-white/5'}`}>副星</div>
                                  {pillars.map((p, i) => (<div key={i} className={`py-4 flex flex-col items-center gap-1 ${isDayMode ? 'bg-gray-50/50' : 'bg-white/5'}`}>{p.data.hiddenStemStars.map((hss, idx) => <span key={idx} className={`text-[10px] ${isDayMode ? 'text-gray-400' : 'text-gray-500'}`}>{hss}</span>)}</div>))}
                                  <div className="py-4 text-gray-500 font-bold flex items-center justify-center">纳音</div>
                                  {pillars.map((p, i) => (<div key={i} className={`py-4 text-[10px] leading-tight ${isDayMode ? 'text-gray-500' : 'text-gray-400'}`}>{p.data.naYin}</div>))}
                                  <div className={`py-4 text-gray-500 font-bold flex items-center justify-center ${isDayMode ? 'bg-gray-50/50' : 'bg-white/5'}`}>长生</div>
                                  {pillars.map((p, i) => (<div key={i} className={`py-4 text-xs bg-gray-50/50 ${isDayMode ? 'text-mystic-gold font-medium' : 'text-mystic-gold/80'}`}>{p.data.xingYun || '-'}</div>))}
                                  <div className="py-4 text-gray-500 font-bold flex items-center justify-center">神煞</div>
                                  {pillars.map((p, i) => (
                                      <div key={i} className="py-4 px-1 flex flex-wrap justify-center items-center content-center gap-1.5 min-w-0 max-h-32 overflow-y-auto scrollbar-hide">
                                          {p.data.shenSha.map((ss, idx) => (
                                              <span key={idx} className={`text-[10px] border px-2 py-0.5 rounded-[4px] whitespace-nowrap leading-none transition-all hover:scale-110 cursor-default ${isDayMode ? 'text-mystic-gold border-mystic-gold/30 bg-mystic-gold/5' : 'text-mystic-gold/80 border-mystic-gold/20 bg-white/5'}`}>
                                                {ss}
                                              </span>
                                          ))}
                          ...
                          {/* Slicing View for Wide Screens */}
                          <div className="space-y-6 px-2 pb-6">
                             <div>
                               <div className="flex items-center">
                                   <div className={`w-10 text-xs font-serif font-bold border-r mr-3 flex flex-col items-center shrink-0 transition-colors ${isDayMode ? 'text-gray-400 border-gray-100' : 'text-gray-500 border-white/10'}`}><span>大</span><span>运</span></div>
                                   <div ref={daYunListRef} className="flex-1 flex overflow-x-auto gap-3 py-2 min-w-0 scrollbar-hide flex-nowrap touch-pan-x">
                                       {chart.daYun.map((yun, i) => (
                                           <button key={i} onClick={() => { setSelectedDaYunIndex(i); setSelectedLiuNianIndex(0); setSelectedLiuYueIndex(0); }} className={`flex-shrink-0 flex flex-col items-center justify-center min-w-[4rem] h-16 rounded-xl border transition-all ${selectedDaYunIndex === i ? 'bg-mystic-gold text-black border-mystic-gold shadow-lg scale-105 z-10' : (isDayMode ? 'bg-white border-gray-100 text-gray-500 hover:bg-gray-50' : 'bg-white/5 border-white/5 text-gray-500')}`}>
                                               <span className="text-base font-serif font-bold">{yun.ganZhi}</span>
                             ...
                             <div>
                               <div className="flex items-center">
                                   <div className={`w-10 text-xs font-serif font-bold border-r mr-3 flex flex-col items-center shrink-0 transition-colors ${isDayMode ? 'text-gray-400 border-gray-100' : 'text-gray-500 border-white/10'}`}><span>流</span><span>年</span></div>
                                   <div ref={liuNianListRef} className="flex-1 flex overflow-x-auto gap-3 py-2 min-w-0 scrollbar-hide flex-nowrap touch-pan-x">
                                       {selectedDaYun?.liuNian.map((ln, i) => (
                                           <button key={i} onClick={() => { setSelectedLiuNianIndex(i); setSelectedLiuYueIndex(0); }} className={`flex-shrink-0 flex flex-col items-center justify-center min-w-[4rem] h-16 rounded-xl border transition-all ${selectedLiuNianIndex === i ? 'bg-mystic-gold text-black border-mystic-gold shadow-lg scale-105 z-10' : (isDayMode ? 'bg-white border-gray-100 text-gray-500 hover:bg-gray-50' : 'bg-white/5 border-white/5 text-gray-500')}`}>
                                               <span className="text-base font-serif font-bold">{ln.ganZhi}</span>
                                               <span className="text-[10px] opacity-70">{ln.year}</span>
                                           </button>
                                       ))}
                                   </div>
                               </div>
                             ...
                             {selectedLiuNian && (
                                <div>
                                  <div className="flex items-center">
                                      <div className={`w-10 text-xs font-serif font-bold border-r mr-3 flex flex-col items-center shrink-0 transition-colors ${isDayMode ? 'text-gray-400 border-gray-100' : 'text-gray-500 border-white/10'}`}><span>流</span><span>月</span></div>
                                      <div ref={liuYueListRef} className="flex-1 flex overflow-x-auto gap-3 py-2 min-w-0 scrollbar-hide flex-nowrap touch-pan-x">
                                          {selectedLiuNian.liuYue.map((m, i) => (
                                              <button key={i} onClick={() => setSelectedLiuYueIndex(i)} className={`flex-shrink-0 flex flex-col items-center justify-center min-w-[4rem] h-16 rounded-xl border transition-all ${selectedLiuYueIndex === i ? 'bg-mystic-gold text-black border-mystic-gold shadow-lg scale-105 z-10' : (isDayMode ? 'bg-white border-gray-100 text-gray-500 hover:bg-gray-50' : 'bg-white/5 border-white/5 text-gray-500')}`}>
                                                  <span className={`text-base font-serif font-bold ${selectedLiuYueIndex === i ? '' : getElementColor(m.ganZhi, isDayMode)}`}>{m.ganZhi}</span>
                                                  <span className="text-[10px] opacity-70">{m.month}</span>
                                              </button>
                                          ))}
                                      </div>
                                  </div>
          ...
          {/* Messages with Responsive Padding */}
          <div className="px-4 sm:px-8 space-y-6 max-w-4xl mx-auto">
             {messages.map((msg) => (
                <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} animate-fade-in-up`}>
                   {msg.role === 'assistant' && (
                     <div className={`w-10 h-10 rounded-full border mr-3 shrink-0 overflow-hidden transition-colors ${isDayMode ? 'bg-white border-gray-100 shadow-sm' : 'border-mystic-gold/30'}`}>
                        <img src={assets.sage_avatar} className="w-full h-full object-cover" alt="Sage" />
                     </div>
                   )}
                   <div className={`max-w-[85%] px-5 py-4 rounded-3xl text-sm leading-relaxed border transition-colors shadow-sm ${
                      msg.role === 'user' 
             ...
             {chatLoading && (
               <div className="flex justify-start">
                 <div className={`w-10 h-10 rounded-full border mr-3 flex items-center justify-center transition-colors ${isDayMode ? 'bg-white border-gray-100' : 'border-mystic-gold/30'}`}>
                    <span className="animate-spin text-mystic-gold text-lg">☯</span>
                 </div>
                 <div className={`px-5 py-4 rounded-3xl text-sm border transition-colors ${isDayMode ? 'bg-white border-gray-100 text-gray-400' : 'bg-mystic-paper/80 border-white/10 text-gray-400'}`}>正在推演乾坤...</div>
               </div>
             )}
          </div>
      </div>

      {/* Input Area (Centered and Wider on PC) */}
      <div className={`shrink-0 w-full px-4 pt-6 pb-6 sm:pb-8 z-20 border-t shadow-[0_-10px_30px_rgba(0,0,0,0.05)] transition-colors ${isDayMode ? 'bg-white border-gray-100' : 'bg-mystic-dark border-white/5'}`}>
            <div className="max-w-4xl mx-auto">
                    ...
                    <div className="flex flex-wrap gap-2 mb-4 animate-fade-in-up">
                        {suggestions.map((s, idx) => (
                            <button 
                              ...
                              className={`py-2 px-4 rounded-full text-xs transition-all shadow-sm active:scale-95 border ${isDayMode ? 'bg-mystic-gold/5 text-mystic-gold border-mystic-gold/20 hover:bg-mystic-gold/10' : 'bg-white/5 text-mystic-gold border-white/10 hover:bg-white/10'}`}
                            >
                        ...
                        {suggestions.length === 0 && !chatLoading && messages.length === 0 && (
                            <button 
                              ...
                              className={`w-full py-4 rounded-2xl font-bold transition-all shadow-xl active:scale-95 flex items-center justify-center gap-3 ${isDayMode ? 'bg-mystic-gold text-white border border-mystic-gold' : 'bg-gradient-to-r from-[#c5b078] to-[#a08d55] text-black'}`}
                            >
                              <IconMagic className={`w-5 h-5 ${isDayMode ? 'brightness-200' : ''}`} />
                              <span className="text-base tracking-[0.2em]">开启专业详盘深度析命</span>
                            </button>
                        )}
                    </div>
                )}
                <div className="relative">
                   <input 
                     ...
                     placeholder="阁下请直言，命理万千皆在其中..." 
                     className={`w-full pl-6 pr-16 py-4 rounded-2xl border outline-none shadow-sm transition-all text-base ${isDayMode ? 'bg-gray-50 border-gray-200 text-gray-900 focus:bg-white focus:border-mystic-gold/50' : 'bg-[#1a1b1e] text-gray-200 border-white/10 focus:border-mystic-gold'}`} 
                   />
                   <button 
                     onClick={() => handleSendMessage()} 
                     disabled={chatLoading} 
                     className="absolute right-3 top-1/2 -translate-y-1/2 w-10 h-10 flex items-center justify-center rounded-xl bg-mystic-gold/20 text-mystic-gold hover:bg-mystic-gold hover:text-white transition-all shadow-lg"
                   >
                     ➤
                   </button>
                </div>
